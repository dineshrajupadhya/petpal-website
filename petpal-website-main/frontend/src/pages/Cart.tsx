    ...
    return (
      <div className="min-h-screen bg-gray-50 py-12 px-4">
        <div className="max-w-md mx-auto bg-white rounded-2xl shadow-xl p-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Log In to View Cart</h1>
          <p className="text-gray-600 mb-8">
            Your cart items have been saved. Please log in to continue checkout.
          </p>
          <div className="space-y-3">
            <Link
              to="/login"
              className="block w-full bg-blue-600 text-white py-3 px-6 rounded-xl hover:bg-blue-700 transition-all font-medium"
            >
              Log In
            </Link>
            <Link
              to="/register"
              className="block w-full border border-gray-300 text-gray-700 py-3 px-6 rounded-xl hover:bg-gray-50 transition-all font-medium"
            >
              Create Account
            </Link>
          </div>
          <p className="text-sm text-gray-500 mt-6">
            Items will be waiting for you after login
          </p>
        </div>
      </div>
    );
...
{('breed' in cartItem.item && cartItem.item.breed) && (
                      ...
                      <div className="text-right">
                        <p className="font-semibold text-gray-900">
                          Rs {Math.round(getItemPrice(cartItem.item) * cartItem.quantity)}
                        </p>
                        <p className="text-sm text-gray-500">
                          Rs {Math.round(getItemPrice(cartItem.item))} each
                        </p>
                      </div>
...
<Link to="/checkout" className="no-underline"><span>Proceed to Checkout</span></Link>
